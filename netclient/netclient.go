...
func Get(reqUrl string, auth string) (respBodyObj ResponseBody, err error) {
	...
	req.Header.Set(AuthHeaderKey(auth), CliRequestData.AuthToken)
	return handleResp(req)
}
...
func GetNew(reqUrl string, auth string) (respBodyObj ResponseBody, err error) {
	req, err := http.NewRequest("GET", reqUrl, nil)
	if err != nil {
		return
	}
...
func PostNew(reqUrl string, auth string, body interface{}) (respBodyObj ResponseBody, err error) {
	postBody, _ := json.Marshal(body)
	requestBody := bytes.NewBuffer(postBody)
	log.WithFields(log.Fields{
		"url":  reqUrl,
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set(AuthHeaderKey(auth), auth)
	return handleRespNew(req)
}

func handleRespNew(req *http.Request) (respBodyObj ResponseBody, err error) {
	client := &http.Client{}
	resp, err := client.Do(req)
	if err != nil {
		return
